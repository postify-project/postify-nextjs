...
export default function AnalyticsDashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl animate-fade-in font-inter text-slate-800">
      <Suspense
        fallback={
          <div className="fixed inset-0 flex flex-col gap-3 items-center justify-center z-[500] bg-slate-50">
      ...
      >
          ...
          <header className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            ...
            {/* Actions Section (Right End) */}
            <div className="flex items-center gap-2 sm:self-center [&>*:last-child]:flex [&>*:last-child]:items-center [&>*:last-child]:gap-2 [&_button]:!w-auto [&_button]:!py-2 [&_button]:!px-3 [&_button]:!text-xs">
              <button
                onClick={handleRefresh}
                className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 min-w-[130px] text-xs font-semibold text-slate-700 shadow-sm transition-all duration-200 hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900 active:scale-[0.98]"
              >
                <svg
                  width="14"
                  height="14"
            ...
            >
              <span
                className={`h-1.5 w-1.5 rounded-full bg-rose-500 ${
                  activePlatform === "youtube" ? "animate-pulse" : "hidden"
                }`}
              />
              YouTube
            </button>
            ...
            >
              <span
                className={`h-1.5 w-1.5 rounded-full bg-blue-500 ${
                  activePlatform === "facebook" ? "animate-pulse" : "hidden"
                }`}
              />
              Facebook
            </button>
