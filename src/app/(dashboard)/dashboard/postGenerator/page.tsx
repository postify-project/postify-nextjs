...
export default function PostGeneratorPage() {
  ...
  const [platform] = useState<string>("Instagram");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await api.post(`${BACKEND}/ai/post`, {
        topic: context,
        platform: platform,
        tone: "Professional",
      });
      ...
      if (res.data?.success && res.data?.data) {
        ...
        setGeneratedPost({
          imageTopic: context,
          caption: d.caption || "",
          hashtags: Array.isArray(d.hashtags) ? d.hashtags : [d.hashtags].filter(Boolean),
          imageUrl: d.image_url || "",
          callToAction: d.call_to_action || "",
        });
      } else {
        throw new Error(res.data?.message || "Failed to generate AI post");
      }
    } catch (err: unknown) {
      const errObj = err as { response?: { data?: { message?: string } }; message?: string };
      console.error("AI Post Generation error:", err);
      setErrorMsg(errObj.response?.data?.message || errObj.message || "AI Post generation failed");
    } finally {
      ...
      {generatedPost && (
        <section className="animate-fade-in rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xl">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-[1fr_1.2fr]">
            ...
            <div className="relative aspect-square overflow-hidden rounded-xl border border-slate-200/80 shadow-sm">
              {generatedPost.imageUrl ? (
                <img
                  src={generatedPost.imageUrl}
                  alt={generatedPost.imageTopic}
